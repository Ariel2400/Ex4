...
void ParallelServer::attend_client_queue() {
    while (is_server_open) {
        ...
        if (client_socket != NO_SOCKET) {
            ...
            std::thread client_input(&ParallelServer::read_from_client, 
                                      this, client_socket, &is_active);
...
void ParallelServer::open(int port) {
    ...
    client_handling_loop = std::make_unique<std::thread>(&ParallelServer::look_for_clients, 
                                                            this, address);
    is_server_open = true;
