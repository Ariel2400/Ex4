...
public:
  ...
  AstarStep(uint32_t new_row, uint32_t new_column, std::string new_path, double new_g) {
    this->row = new_row;
  ...
  void setHeuristic(uint32_t end_row, uint32_t end_column) {
    this->h = MIN_STEP_WEIGHT * (std::abs(static_cast<int>(end_row) - static_cast<int>(this->row)) +
                                 std::abs(static_cast<int>(end_column) - static_cast<int>(this->column)));
