...
BestFSMatrixSearcher &BestFSMatrixSearcher::operator=(const BestFSMatrixSearcher &other) {
    return *this;
}

void BestFSMatrixSearcher::update_in_queue(std::priority_queue<Step, std::vector<Step>, CompareWeight>* queue, Step newStep) {
...
SearchStatus BestFSMatrixSearcher::search(const Problem &problem, std::string* solution, int* weight) {
    ...
    while (!open.empty()) {
        ...
        if (step.row - 1 >= 0) {
            successors.push_back(Step(step.row - 1, step.column, step.path + "Up,", step.weight + matrix->get_value(step.row - 1, step.column)));
        }
        if (step.row + 1 < height) {
            successors.push_back(Step(step.row + 1, step.column, step.path + "Down,", step.weight + matrix->get_value(step.row + 1, step.column)));
        }
        if (step.column - 1 >= 0) {
            successors.push_back(Step(step.row, step.column - 1, step.path + "Left,", step.weight + matrix->get_value(step.row, step.column - 1)));
        }
        if (step.column + 1 < width) {
            successors.push_back(Step(step.row, step.column + 1, step.path + "Right,", step.weight + matrix->get_value(step.row, step.column + 1)));
        }
        for (int i = 0; i < successors.size(); ++i) {
            if (successors.at(i).row >= 0 && successors.at(i).row < height 
                && successors.at(i).column >= 0 && successors.at(i).column < width 
                && matrix->get_value(successors.at(i).row, successors.at(i).column) != BLOCK
...
/*
int main() {
    std::unique_ptr<BestFSMatrixSearcher> searcher = std::make_unique<BestFSMatrixSearcher>();
    Problem problem = {std::make_unique<Matrix>(3, 3), 0, 0, 2, 0};
