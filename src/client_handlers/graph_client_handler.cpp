...
void GraphClientHandler::handle_client(int client_socket, std::string algorithm) {
  ...
  if (problem_description.length() < two_line_breaks.length() 
      || problem_description.substr(problem_description.length() 
      - two_line_breaks.length(), two_line_breaks.length()).compare(two_line_breaks)) {
  ...
  std::sregex_token_iterator line_iter(problem_description.begin(), 
                                        problem_description.end(), line_end_regex, -1);
  ...
  uint32_t width = std::stoi(lines.at(0).substr(dimensions_comma_index + 1, 
                                              lines.at(0).length() - dimensions_comma_index - 1));
  ...
  for (uint32_t i = 0; i < height; ++i) {
    ...
    std::regex comma(",");
    std::sregex_token_iterator value_iter(lines.at(i + 1).begin(), 
                                          lines.at(i + 1).end(), comma, -1);
  ...
  uint32_t start_column = std::stoi(lines.at(height + 1).substr(start_comma_index + 1,
                                    lines.at(0).length() - start_comma_index - 1));
  ...
  uint32_t end_column = std::stoi(lines.at(height + 2).substr(end_comma_index + 1, 
                                    lines.at(0).length() - end_comma_index - 1));
...
void GraphClientHandler::send_response(int client_socket, SearchStatus status, 
                                      std::string path, double weight, std::string algorithm) {
